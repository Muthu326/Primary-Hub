...
if not st.session_state.authenticated:
    ...
    with col2:
        ...
        with tab2:
            ...
            # Division Selection - Foolproof Reset Logic
            sch_col, gr_col = st.columns(2)
            with sch_col:
                sch_type = st.radio("School Division", ["Primary", "High School"], horizontal=True, key="sch_type_radio")
            with gr_col:
                # Calculate grades and use a dynamic KEY to force widget reset
                grade_list = [1, 2, 3, 4, 5] if sch_type == "Primary" else [6, 7, 8, 9, 10]
                new_grade = st.selectbox("Grade / வகுப்பு", grade_list, key=f"grade_sel_{sch_type}")
