...
if not st.session_state.authenticated:
    ...
    with col2:
        ...
        with tab1:
            token_input = st.text_input(B_LANG['enter_token'], placeholder="e.g. STUD001", key="login_tk")
            if st.button("Enter / நுழைய", key="enter_btn"):
                user = validate_token(token_input.upper())
                if user:
                    name, grade, status = user
                    if status in ['active', 'admin']:
                        st.session_state.authenticated = True
                        st.session_state.user_data = {'name': name, 'grade': grade, 'status': status, 'token': token_input.upper()}
                        st.rerun()
                    elif status == 'pending':
